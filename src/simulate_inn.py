...
def simulate(disturbance_enabled=True, T_final=60.0, trajectory_type="spline"):
    ...
    controller = INNController(model, trajectory_shape=trajectory_type, T_final=T_final)
    ...
    # Neural Network Weights State (hidden_dim x output_dim)
    W0 = controller.W_nominal.flatten()
    
    # State Vector: [X(8), W(200)]
    Z0 = np.hstack((X0, W0))

    def closed_loop_dynamics(t, Z):
        X = Z[0:8]
        W_flat = Z[8:]
        W = W_flat.reshape(controller.hidden_dim, controller.output_dim)

        ref = reference_trajectory(t, shape=trajectory_type, T_final=T_final)

        # 1. Compute Control
        tau = controller.compute_control(t, X, W, ref)
        ...
        W_dot = controller.adaptation_dynamics(X, W, ref)

        return np.hstack((X_dot, W_dot.flatten()))
    ...
    for i, ti in enumerate(t):
        ref = refs[i]
        W = W_history[i, :].reshape(controller.hidden_dim, controller.output_dim)
        tau[i, :] = controller.compute_control(ti, X[i, :], W, ref)
...
def plot_results(result, mode_name):
    ...
    ax.plot(eta[:, 0], eta[:, 1], eta[:, 2], linewidth=2.0, color="green", label=f"INN Path")
    ax.set_xlabel("X (m)", labelpad=10)
    ax.set_ylabel("Y (m)", labelpad=10)
    ax.set_zlabel("Z (m)", labelpad=10)
    ax.set_title(f"INN 4-DOF Tracking ({mode_name})")
    ax.legend()
    fig.tight_layout()
    fig.savefig(f"figures/inn_{mode_name.lower()}_01_3d_trajectory.pdf", format="pdf", dpi=300, bbox_inches="tight")
    plt.close(fig)

    # 2. XY Tracking
    fig, ax = plt.subplots(figsize=(8.0, 5.5))
    ax.plot(eta_d[:, 0], eta_d[:, 1], "--", linewidth=2.5, color="gray", label="Reference")
    ax.plot(eta[:, 0], eta[:, 1], linewidth=2.0, color="green", label=f"INN Path")
    ax.set_xlabel("X (m)")
    ax.set_ylabel("Y (m)")
    ax.set_title(f"INN XY Tracking ({mode_name})")
    ax.legend()
    ax.axis("equal")
    fig.tight_layout()
    fig.savefig(f"figures/inn_{mode_name.lower()}_02_xy_tracking.pdf", format="pdf", dpi=300, bbox_inches="tight")
    plt.close(fig)

    # 3. Error Comparison
    fig, ax = plt.subplots(figsize=(8.0, 5.0))
    ax.plot(t, pos_error, linewidth=2.0, color="purple", label=f"INN Error (RMS={result['e_rms']:.4f}m)")
    ax.set_xlabel("Time (s)")
    ax.set_ylabel("3D Position Error (m)")
    ax.set_title(f"INN Tracking Error ({mode_name})")
    ax.legend()
    fig.tight_layout()
    fig.savefig(f"figures/inn_{mode_name.lower()}_03_error.pdf", format="pdf", dpi=300, bbox_inches="tight")
    plt.close(fig)
    ...
    ax.set_title(f"INN Control Inputs | J={effort:.2f} ({mode_name})")
    ax.legend()
    fig.tight_layout()
    fig.savefig(f"figures/inn_{mode_name.lower()}_04_control.pdf", format="pdf", dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def main():
    ...
    print(f"Running INN Simulation (Trajectory: {args.trajectory}, Duration: {args.duration}s)...")
    ...
    save_results_csv(nominal, "results/inn_nominal.csv")
    save_results_csv(disturbed, "results/inn_disturbed.csv")
    ...
    summary.to_csv("results/inn_summary.csv", index=False)

    print("\nINN Simulation Complete")
    print(summary)
