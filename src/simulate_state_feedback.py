...
def simulate(disturbance_enabled=True, T_final=60.0, trajectory_type="spline"):
    ...
    controller = StateFeedbackController(model, trajectory_shape=trajectory_type, T_final=T_final)
    ...
    # Start on the reference trajectory to evaluate tracking performance,
    # not initial-condition recovery.
    ref0 = reference_trajectory(0.0, shape=trajectory_type, T_final=T_final)
    eta0 = ref0["eta_d"]
    # Pass a dummy state for initialization (velocity is feedforward-only anyway)
    ...
    eta_dot_d = np.array([ref["eta_dot_d"] for ref in refs])
    ...
    for i, ti in enumerate(t):
        ref = reference_trajectory(ti, shape=trajectory_type, T_final=T_final)
        tau[i, :] = controller.compute_control(ti, X[i, :], ref)
...
def plot_results(result, mode_name):
    ...
    ax.set_title(f"State Feedback 4-DOF Tracking ({mode_name})")
    ax.legend()
    fig.tight_layout()
    fig.savefig(f"figures/state_feedback_{mode_name.lower()}_01_3d_trajectory.pdf", format="pdf", dpi=300, bbox_inches="tight")
    plt.close(fig)
    ...
    ax.set_xlabel("X (m)")
    ax.set_ylabel("Y (m)")
    ax.set_title(f"State Feedback XY Tracking ({mode_name})")
    ax.legend()
    ax.axis("equal")
    fig.tight_layout()
    fig.savefig(f"figures/state_feedback_{mode_name.lower()}_02_xy_tracking.pdf", format="pdf", dpi=300, bbox_inches="tight")
    plt.close(fig)
    ...
    ax.set_title(f"State Feedback Tracking Error ({mode_name})")
    ax.legend()
    fig.tight_layout()
    fig.savefig(f"figures/state_feedback_{mode_name.lower()}_03_error.pdf", format="pdf", dpi=300, bbox_inches="tight")
    plt.close(fig)
    ...
    ax.set_title(f"State Feedback Control Inputs | J={effort:.2f} ({mode_name})")
    ax.legend()
    fig.tight_layout()
    fig.savefig(f"figures/state_feedback_{mode_name.lower()}_04_control.pdf", format="pdf", dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def main():
    ...
    save_results_csv(nominal, "results/state_feedback_nominal.csv")
    save_results_csv(disturbed, "results/state_feedback_disturbed.csv")
    ...
    summary.to_csv("results/state_feedback_summary.csv", index=False)

    print("\nState Feedback Simulation Complete")
    print(summary)
