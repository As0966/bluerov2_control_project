...
def simulate(disturbance_enabled=True, T_final=60.0, trajectory_type="spline"):
    ...
    # Adaptive State: theta(8) + z_eta(4)
    # Start with 70% of nominal parameters to test adaptation
    theta0 = 0.70 * controller.theta_nominal
    z0 = np.zeros(4)
    
    # State Vector: [X(8), theta(8), z(4)] = 20
    Z0 = np.hstack((X0, theta0, z0))
    ...
    def closed_loop_dynamics(t, Z):
        ...
        # 1. Compute Control
        tau = controller.compute_control(t, X, theta, z_eta, ref)
        ...
        # 3. Adaptation & Integral Dynamics
        nu_d = controller.desired_body_velocity(X, ref, z_eta)
        nu_d_dot = controller.desired_body_acceleration(X, ref, z_eta)
        
        theta_dot = controller.adaptation_dynamics(X, nu_d, nu_d_dot, theta)
        
        e_eta = X[0:4] - ref["eta_d"]
        e_eta[3] = controller.wrap_angle(e_eta[3])
        z_eta_dot = e_eta

        return np.hstack((X_dot, theta_dot, z_eta_dot))
    ...
    for i, ti in enumerate(t):
        ref = refs[i]
        tau[i, :] = controller.compute_control(ti, X[i, :], theta_hist[i,:], z_hist[i,:], ref)
...
def save_results_csv(results, filename):
    ...
    df = pd.DataFrame({
        "t": t,
        "x": X[:, 0], "y": X[:, 1], "z": X[:, 2], "psi": X[:, 3],
        "x_d": eta_d[:, 0], "y_d": eta_d[:, 1], "z_d": eta_d[:, 2], "psi_d": eta_d[:, 3],
        "e_x": e_eta[:, 0], "e_y": e_eta[:, 1], "e_z": e_eta[:, 2], "e_psi": e_eta[:, 3],
        "tau_u": tau[:, 0], "tau_v": tau[:, 1], "tau_w": tau[:, 2], "tau_r": tau[:, 3],
        "m_u_hat": theta[:, 0], "d_u_hat": theta[:, 4]
    })
...
def plot_results(result, mode_name):
    ...
    ax.plot(eta[:, 0], eta[:, 1], eta[:, 2], linewidth=2.0, color="orange", label=f"AdaBS Path")
    ...
    ax.legend()
    fig.tight_layout()
    fig.savefig(f"figures/adabs_{mode_name.lower()}_01_3d_trajectory.pdf", format="pdf", dpi=300, bbox_inches="tight")
    plt.close(fig)

    # 2. Parameter Convergence
    fig, ax = plt.subplots(figsize=(8.0, 5.5))
    ax.plot(t, theta[:, 0], label="Mass U (hat)")
    ax.plot(t, theta[:, 4], label="Damping U (hat)")
    ax.set_xlabel("Time (s)")
    ax.set_ylabel("Estimate Value")
    ax.set_title(f"Parameter Convergence ({mode_name})")
    ax.legend()
    fig.tight_layout()
    fig.savefig(f"figures/adabs_{mode_name.lower()}_02_params.pdf", format="pdf", dpi=300, bbox_inches="tight")
    plt.close(fig)

    # 3. Error Comparison
    fig, ax = plt.subplots(figsize=(8.0, 5.0))
    ax.plot(t, pos_error, linewidth=2.0, color="blue", label=f"AdaBS Error (RMS={result['e_rms']:.4f}m)")
    ax.set_xlabel("Time (s)")
    ax.set_ylabel("3D Position Error (m)")
    ax.set_title(f"Adaptive Backstepping Tracking Error ({mode_name})")
    ax.legend()
    fig.tight_layout()
    fig.savefig(f"figures/adabs_{mode_name.lower()}_03_error.pdf", format="pdf", dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def main():
    ...
    print(f"Running Adaptive Backstepping Simulation (Trajectory: {args.trajectory}, Duration: {args.duration}s)...")
    ...
    save_results_csv(nominal, "results/adabs_nominal.csv")
    save_results_csv(disturbed, "results/adabs_disturbed.csv")
    ...
    summary = pd.DataFrame([
        {
            "controller": "Adaptive Backstepping",
            "case": "nominal",
    ...
    summary.to_csv("results/adabs_summary.csv", index=False)

    print("\nAdaptive Backstepping Simulation Complete")
    print(summary)
