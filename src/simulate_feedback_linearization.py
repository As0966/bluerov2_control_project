...
from bluerov2_control.controllers.feedback_linearization import FeedbackLinearizationController


def simulate(disturbance_enabled=True, T_final=60.0, trajectory_type="spline"):
    model = BlueROV2Model4DOF()
    controller = FeedbackLinearizationController(model, trajectory_shape=trajectory_type, T_final=T_final)
...
def plot_results(result, mode_name):
    ...
    ax.set_title(f"Feedback Linearization 4-DOF Tracking ({mode_name})")
    ax.legend()
    fig.tight_layout()
    fig.savefig(f"figures/feedback_linearization_{mode_name.lower()}_01_3d_trajectory.pdf", format="pdf", dpi=300, bbox_inches="tight")
    plt.close(fig)
    ...
    ax.set_xlabel("X (m)")
    ax.set_ylabel("Y (m)")
    ax.set_title(f"Feedback Linearization XY Tracking ({mode_name})")
    ax.legend()
    ax.axis("equal")
    fig.tight_layout()
    fig.savefig(f"figures/feedback_linearization_{mode_name.lower()}_02_xy_tracking.pdf", format="pdf", dpi=300, bbox_inches="tight")
    plt.close(fig)
    ...
    ax.set_title(f"Feedback Linearization Tracking Error ({mode_name})")
    ax.legend()
    fig.tight_layout()
    fig.savefig(f"figures/feedback_linearization_{mode_name.lower()}_03_error.pdf", format="pdf", dpi=300, bbox_inches="tight")
    plt.close(fig)
    ...
    ax.set_title(f"Feedback Linearization Control Inputs | J={effort:.2f} ({mode_name})")
    ax.legend()
    fig.tight_layout()
    fig.savefig(f"figures/feedback_linearization_{mode_name.lower()}_04_control.pdf", format="pdf", dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def main():
    ...
    save_results_csv(nominal, "results/fl_nominal.csv")
    save_results_csv(disturbed, "results/fl_disturbed.csv")
    ...
    summary.to_csv("results/fl_summary.csv", index=False)

    print("\nFeedback Linearization Simulation Complete")
    print(summary)
