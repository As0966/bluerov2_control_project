...
from bluerov2_control.controllers.observer import NonlinearObserverBasedController


def simulate(disturbance_enabled=True, T_final=60.0, trajectory_type="spline"):
    model = BlueROV2Model4DOF()
    controller = NonlinearObserverBasedController(model, trajectory_shape=trajectory_type, T_final=T_final)
    ...
    # Observer starts close but not perfect.
    # This tests convergence while avoiding artificial large initial failure.
    # Observer starts close but not perfect.
    Xhat0 = X0.copy()
    Xhat0[4:8] = 0.9 * X0[4:8]

    # Bias estimation starts at zero
    bias0 = np.zeros(4)
    
    # State Vector: [X(8), Xhat(8), bias(4)]
    Z0 = np.hstack((X0, Xhat0, bias0))
    ...
    def closed_loop_dynamics(t, Z):
        ...
        Xhat_dot, bias_dot = controller.observer_dynamics(t, Xhat, y_meas, tau, bias_hat=bias_hat)

        return np.hstack((X_dot, Xhat_dot, bias_dot))
...
def plot_results(result, mode_name):
    ...
    ax.set_title(f"Observer 4-DOF Tracking ({mode_name})")
    ax.legend()
    fig.tight_layout()
    fig.savefig(f"figures/observer_{mode_name.lower()}_01_3d_trajectory.pdf", format="pdf", dpi=300, bbox_inches="tight")
    plt.close(fig)
    ...
    ax.set_xlabel("X (m)")
    ax.set_ylabel("Y (m)")
    ax.set_title(f"Observer XY Tracking ({mode_name})")
    ax.legend()
    ax.axis("equal")
    fig.tight_layout()
    fig.savefig(f"figures/observer_{mode_name.lower()}_02_xy_tracking.pdf", format="pdf", dpi=300, bbox_inches="tight")
    plt.close(fig)
    ...
    ax.set_title(f"Observer Tracking Error ({mode_name})")
    ax.legend()
    fig.tight_layout()
    fig.savefig(f"figures/observer_{mode_name.lower()}_03_error.pdf", format="pdf", dpi=300, bbox_inches="tight")
    plt.close(fig)
    ...
    ax.set_title(f"Observer Control Inputs | J={effort:.2f} ({mode_name})")
    ax.legend()
    fig.tight_layout()
    fig.savefig(f"figures/observer_{mode_name.lower()}_04_control.pdf", format="pdf", dpi=300, bbox_inches="tight")
    plt.close(fig)
    ...
    ax.set_title(f"Observer State Estimation Error ({mode_name})")
    ax.set_xlabel("Time (s)")
    ax.legend(ncol=2)
    fig.tight_layout()
    fig.savefig(f"figures/observer_{mode_name.lower()}_05_estimation.pdf", format="pdf", dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def main():
    ...
    save_results_csv(nominal, "results/nonlinear_observer_nominal.csv")
    save_results_csv(disturbed, "results/nonlinear_observer_disturbed.csv")
    ...
    summary = pd.DataFrame([
        {
            "controller": "Nonlinear Observer-Based",
            "case": "nominal",
    ...
    summary.to_csv("results/nonlinear_observer_summary.csv", index=False)

    print("\nNonlinear Observer-Based Simulation Complete")
    print(summary)
